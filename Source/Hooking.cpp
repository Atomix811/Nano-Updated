...
fpFileRegister		Hooking::m_fileregister;
...
{
	bool g_LogScriptEvents; // just for logg events
...
bool SC_GET_EVENT_DATA(std::int32_t event_Group, std::int32_t event_Index, std::int64_t* args, std::uint32_t arg_Count)
{
	...
	if (Event_Result && Protections::g_LogScriptEvents) // might but this in a GetTickCount() loop if its lagging or causing crashes
	{
		Log::Msg("\n<===========================================================================>");
...
void Hooking::FindPatterns()
{
	...
	Hooking::m_fileregister = (fpFileRegister)(Memory::pattern("48 89 5C 24 ? 48 89 6C 24 ? 48 89 7C 24 ? 41 54 41 56 41 57 48 83 EC 50 48 8B EA 4C 8B FA 48 8B D9 4D 85 C9").count(1).get(0).get<decltype(Hooking::m_fileregister)>());
...
void __declspec(noreturn) Hooking::Cleanup()
{
	...
	if (PathFileExistsA(DeleteP.c_str()))
	...
	fclose(stdout);
	FreeConsole();
	//MessageBoxA(NULL, Mod_Menu_Name " UNLOADED!", "UNLOADED!", MB_ICONHAND);
